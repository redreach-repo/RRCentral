...
------------------------------------------------------------
-- VENDORS (suppliers — uniforms, COGS, etc.)
------------------------------------------------------------
...
create policy "Authenticated users full access" on vendors for all using (auth.role() = 'authenticated') with check (auth.role() = 'authenticated');
